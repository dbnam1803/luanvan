    <!-- Slide -->
    
    <section class="slider-section">
      <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-inner">
          <div class="carousel-item active">
            <img src="./customer/assets/img/slide.webp" class="d-block w-100" alt="slider1" />
          </div>
          <div class="carousel-item">
            <img src="./customer/assets/img/slide2.webp" class="d-block w-100" alt="slider2" />
          </div>
          <div class="carousel-item">
            <img src="./customer/assets/img/slide_2_img.webp" class="d-block w-100" alt="slider3" />
          </div>
        </div>
        <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls"
          data-bs-slide="prev">
          <span class="carousel-control-prev-icon" aria-hidden="true"></span>
          <!-- <span class="visually-hidden">Previous</span> -->
        </button>
        <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls"
          data-bs-slide="next">
          <span class="carousel-control-next-icon" aria-hidden="true"></span>
          <!-- <span class="visually-hidden">Next</span> -->
        </button>
      </div>
      <div class="my-cointainer">
        <div class="row d-flex">
          <div class="col-6"></div>
          <div class="col-6"></div>
        </div>
      </div>
    </section>

      <!-- Banner Collection  -->

    <section class="banner-section section-padding">
      <div class="my-container">
        <div class="row mb--28">
          <div class="col-5 mb-28">
            <div class="banner-items">
              <a href="/shop" class="banner-items-thumbnail position-relative">
                <img alt="" src="./customer/assets/img/banner1.png" />
                <div class="banner-items-content position-absolute">
                  <span class="banner-items-content-subtitle">Giảm giá 25%</span>
                  <h2 class="banner-items-content-title">
                    Bộ sưu tập mùa xuân
                  </h2>
                  <span class="banner-items-content-link">Xem giảm giá
                    <svg class="banner-items-content-arrow-icon" xmlns="http://www.w3.org/2000/svg" width="20.2"
                      height="12.2" viewBox="0 0 6.2 6.2">
                      <path d="M7.1,4l-.546.546L8.716,6.713H4v.775H8.716L6.554,9.654,7.1,10.2,9.233,8.067,10.2,7.1Z"
                        transform="translate(-4 -4)" fill="currentColor"></path>
                    </svg>
                  </span>
                </div>
              </a>
            </div>
          </div>
          <div class="col-7">
            <div class="row">
              <div class="col-6 mb-28">
                <div class="banner-items">
                  <a href="/shop" class="banner-items-thumbnail position-relative">
                    <img alt="" src="./customer/assets/img/banner2.png" />
                    <div class="banner-items-content position-absolute">
                      <span class="banner-items-content-subtitle">Giảm giá 25%</span>
                      <h2 class="banner-items-content-title">
                      Bộ sưu tập mùa xuân
                      </h2>
                      <span class="banner-items-content-link">Xem giảm giá
                        <svg class="banner-items-content-arrow-icon" xmlns="http://www.w3.org/2000/svg" width="20.2"
                          height="12.2" viewBox="0 0 6.2 6.2">
                          <path d="M7.1,4l-.546.546L8.716,6.713H4v.775H8.716L6.554,9.654,7.1,10.2,9.233,8.067,10.2,7.1Z"
                            transform="translate(-4 -4)" fill="currentColor"></path>
                        </svg>
                      </span>
                    </div>
                  </a>
                </div>
              </div>
              <div class="col-6 mb-28">
                <div class="banner-items">
                  <a href="/shop" class="banner-items-thumbnail position-relative">
                    <img alt="" src="./customer/assets/img/banner3.png" />
                    <div class="banner-items-content position-absolute">
                      <span class="banner-items-content-subtitle">Giảm giá 25%</span>
                      <h2 class="banner-items-content-title">
                      Bộ sưu tập mùa xuân
                      </h2>
                      <span class="banner-items-content-link">Xem giảm giá
                        <svg class="banner-items-content-arrow-icon" xmlns="http://www.w3.org/2000/svg" width="20.2"
                          height="12.2" viewBox="0 0 6.2 6.2">
                          <path d="M7.1,4l-.546.546L8.716,6.713H4v.775H8.716L6.554,9.654,7.1,10.2,9.233,8.067,10.2,7.1Z"
                            transform="translate(-4 -4)" fill="currentColor"></path>
                        </svg>
                      </span>
                    </div>
                  </a>
                </div>
              </div>
              <div class="banner-items">
                <a href="/shop" class="banner-items-thumbnail position-relative">
                  <img alt="" src="./customer/assets/img/banner4.png" />
                  <div class="banner-items-content position-absolute">
                    <span class="banner-items-content-subtitle">Giảm giá 25%</span>
                    <h2 class="banner-items-content-title">
                    Bộ sưu tập mùa xuân
                    </h2>
                    <span class="banner-items-content-link">Xem giảm giá
                      <svg class="banner-items-content-arrow-icon" xmlns="http://www.w3.org/2000/svg" width="20.2"
                        height="12.2" viewBox="0 0 6.2 6.2">
                        <path d="M7.1,4l-.546.546L8.716,6.713H4v.775H8.716L6.554,9.654,7.1,10.2,9.233,8.067,10.2,7.1Z"
                          transform="translate(-4 -4)" fill="currentColor"></path>
                      </svg>
                    </span>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>